...
- Follow the `AGENTS.md` workflow loop: context → plan → plan review → TDD →
  verify → security review (when the change touches a security surface) → exit
  review.
- Plan and exit reviews run in fresh-context subagents, never in the implementing
  context. The implementing context is never the only authority declaring work
  VERIFIED. Pick the reviewer whose scope matches the change; do not stack
...
- Never edit files under `gen/` — `make gen` owns them.
